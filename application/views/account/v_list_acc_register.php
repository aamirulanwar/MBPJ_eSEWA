...
<!--        </div>-->
    ...
    <div class="card-body">
        <form class="form-horizontal" action="/account/account_create_list" method="post">
            <div class="row">
                            ...
                            <td class="text-center">
                                <?php
                                    ...
                                    elseif ($row['STATUS_APPLICATION']== STATUS_APPLICATION_APPROVED && $row['STATUS_AGREE']==STATUS_AGREE_ACCEPTED && $row['STATUS_CREATE_ACCOUNT']==STATUS_CREATE_ACCOUNT_NO):
                                        if($this->auth->access_view($this->curuser,array(5002))):
                                        ?>
                                            <a class="btn btn-block btn-primary btn-display" href="/account/create_acc/<?php echo urlEncrypt($row['APPLICATION_ID'])?>"><span class="glyphicon glyphicon-edit"></span> Daftar akaun </a>
                                        <?php
                                        endif;
                                    else:
                                        ?>
                                            <a class="btn btn-block btn-secondary btn-display" href="/rental_application/application_process/<?php echo urlEncrypt($row['APPLICATION_ID'])?>"><span class="glyphicon glyphicon-edit"></span> Terperinci </a>
                                        <?php
                                    endif;
                                ?>
                            </td>
